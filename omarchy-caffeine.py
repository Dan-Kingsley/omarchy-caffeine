...
PREV_STATE_FILE = os.path.expanduser(
    "~/.local/state/omarchy/toggles/screensaver-prev-state"
)
...
class OmarchyCaffeine(Gtk.Application):
    def __init__(self):
        Gtk.Application.__init__(
            self,
            application_id="com.omarchy.caffeine",
    ...
    def get_theme_color(self):
        ...
        try:
            with open(THEME_FILE, "r") as f:
                ...
                match = re.search(
                    r"@define-color\s+foreground\s+#([0-9a-fA-F]{6});", content
                )
                if match:
                    ...
                    print(
                        "Could not find foreground color in theme file, using default"
                    )
        except Exception as e:
    ...
    def build_menu(self, active):
        ...
        if active:
            ...
            for label, seconds in durations:
                ...
                mi.connect(
                    "activate", lambda _, s=seconds, l=label: self.start_timer(s, l)
                )
                menu.append(mi)
    ...
    def start_caffeine(self, _, duration=None):
        ...
        # 3. Inhibit idle using Wayland idle-inhibit protocol (same as video players)
        # GTK handles the D-Bus/Wayland protocol automatically
        self.inhibit_cookie = self.inhibit(
            None,
            Gtk.ApplicationInhibitFlags.IDLE,
            "Caffeine is keeping screen awake",
        )
        ...
        msg = (
            f"System will stay awake for {duration}"
            if duration
            else "System will stay awake"
        )
    ...
    def stop_caffeine(self, _):
        ...
        subprocess.run(
            [
                ...
                self.icon_empty,
                "Caffeine Disabled",
                "System will lock normally"
                if not should_exist_state_file
                else "Screensaver remains disabled",
            ]
        )
